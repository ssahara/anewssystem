...
//            msg($delete_record." = |".$delete_anchor.'|',0);
        ...
        } else if( (strlen($post_prefix)>2) && (auth_quickaclcheck($targetpage) < AUTH_EDIT) ) {
        ...
        if ($ans_conf['param']==='author') {
            ...
            foreach ($records as $record) {
                ...
                if (trim($fields[1]) == "textarea") {
                    ...
                    } else if($conf['showuseras'] == 'username') {
                        // real name
                        $default_value = $current_usr['userinfo']['name'];
                    } else if($conf['showuseras'] == 'email') {
                        // ofuscated mail address according mailguard settings
                        $default_value = $current_usr['userinfo']['name'];
                    } else if($conf['showuseras'] == 'email_link') {
                        $default_value = $current_usr['userinfo']['name'];
                    } else $default_value = "";
             ...
             // 2. create preview output
                // split line items into key and data
                 ...
                 foreach ($temp_array as $item) {
                     ...
                     } else if (($key=='stop') && strtotime(trim($value)) > time()) {
                         $bFlag = true;
                     } else if($key=='text'){
                         if($bFlag !== true) break;
                         // replace media links
...
/*                      if (strpos($ans_conf['param'], 'archive')!== false) {
                        ...
                        elseif ($key=='head') {
                            ...
                            $news_date .= ', '. $value;
                        } else if (($key=='tags') && (isset($prefs[1]) !== false)) {
                            $tags = explode(',',$value);
                            foreach ($tags as $tag) {
                    ...
                    if (($aFlag === true) && ($bFlag === true) && ($tag_flag === true)) {
                        ...
                        $item_counter++;
                    } else if (isset($prefs['anchor'])===true) {
