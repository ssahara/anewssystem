...
class action_plugin_anewssystem extends DokuWiki_Action_Plugin {
    ...
    function output(&$event, $param) {
      ...
      /*------- add news action part -----------------------------------------*/
        ...
        // qty   ... limits the number of news headlines starting with most recent
        //           (either integer or all, default:all)
        // tag   ... consider all news where news article owns the given tag string
        //           (empty per default) tag delimiter is "|"
        // style ... css style string as used in HTML (except quotation marks) 
        //           for the outer element div
        // class ... css style for usecase toc, page or box
        // ho    ... headlinesonly will list the news headlines without timestamp 
        //           and author (on/off, default: off)
        ...
        foreach ($newsitems as $article) {
            ...
            foreach ($article_array as $item) {
                ...
                if ($key=='anchor') {
                    ...
                    if (isset($archive_options['date']) &&
                        ($archive_options['date'] !== date('m.Y',strtotime($value)))) break;
                    $aFlag = true;
                } else if ($key=='head') {
                    ...
                    $news_head = '<a href="'.$value.'" id="'.$value.'" name="'.$value.'">'.
                                 trim($news_head) .'</a>'.NL;
                } else if ($key=='author') {
                    $news_date .= ', '. $value;
                } else if (($key=='tags') && (isset($archive_options['tag']) !== false)) {
//                  echo $value.'<br />';
                    $tags = explode(',',$value);
                    foreach ($tags as $tag) {
            ...
            if ((isset($archive_options['tag']) === false) ||
                (strlen($archive_options['tag']) <2)) $tag_flag = true;

            if (($aFlag === true) && ($tag_flag === true)) {
                // stop adding older news articles if quantity is reached
//              echo intval($archive_options['qty']).' >= '.$qty.'<br>';
                $qty++;
                if (($qty > intval($archive_options['qty'])) && 
                    ($archive_options['qty']!=='all')) break;
                ...
                if (($old_year  !== $new_year) &&
                    (($archive_options['class']==='page') || ($archive_options['ho']==='off'))) {
                    if (trim($old_year) !== '') $close_ytag = "</li></ul>".NL;
                    $output .= $close_ytag.'<ul><li class="level1">'.
                               '<div class="li">'.$new_year.'</div><ul class="n_box">';
                    $old_year  = $new_year;
                }
                ...
                if (($old_month  !== $new_month) &&
                    ...
                    $output .= $close_mtag.'<ul><li class="level2">'.
                               '<div class="li">'.$new_month.'</div>';
                    $old_month = $new_month;
                }
                ...
                if (($archive_options['tag']!==false) &&
                    ($archive_options['tag']!=='off') &&
                    ($archive_options['class']=='page')) {
                    $output .= '<div class="archive_item">'.
                               trim($news_date).$news_head.$news_subtitle.'</div>'.NL;
                } else {
                    $output .= '<ul><li class="level3"><div class="li">'.
                               trim($news_date).$news_head.'</div></li></ul>'.NL;
                }
